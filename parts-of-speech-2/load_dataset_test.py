...
# %%
TAG_NAMES = dataset.features["upos"].feature.names
TAG_NAMES
# %%
...
example["tokens"]
# %%
def tokenize_with_labels(text, ud_tokens, ud_labels, ud_heads, tokenizer):
    ...
    for (start, end), token in zip(tokenized["offset_mapping"], tokenized["input_ids"]):
        word = text[start:end]
        print(repr(word))
        print(repr(str(tokenizer.decode([token]))))
...
def upos_to_tag_name(upos_label):
...
tokenized, labels = tokenize_with_labels(
    example["text"],
    example["tokens"],
    example["upos"],
    example["head"],

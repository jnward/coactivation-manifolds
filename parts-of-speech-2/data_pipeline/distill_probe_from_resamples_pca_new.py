...
def train_probe_with_val(X: np.ndarray, Y: np.ndarray, val_X: np.ndarray, val_Y: np.ndarray):
    ...
    if REGULARIZATION_TYPE == "l1":
        cone_optimizer = torch.optim.Adam(cone_model.parameters(), lr=LR)
    else:
        cone_optimizer = torch.optim.AdamW(cone_model.parameters(), lr=LR, weight_decay=REGULARIZATION_STRENGTH)
    ...
    for epoch in range(EPOCHS):
        ...
        for step_idx, (xb, yb) in enumerate(pbar):
            ...
            base_loss = l2_loss(cone_probs, yb)
            loss = base_loss

            # Compute weight sparsity
            with torch.no_grad():
                total_weights = sum(p.numel() for n, p in cone_model.named_parameters() if 'weight' in n)
                zero_weights = sum((p == 0).sum().item() for n, p in cone_model.named_parameters() if 'weight' in n)
                sparsity = zero_weights / total_weights if total_weights > 0 else 0.0

            if REGULARIZATION_TYPE == "l1":
                l1_reg = sum(p.abs().sum() for name, p in cone_model.named_parameters() if 'weight' in name)
